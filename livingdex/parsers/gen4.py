...
def parse(save: Path, sub_parser: str) -> list[list[str]]:
    ...
    with save.open("rb") as f:
        ...
        (slot2_counter1, slot2_counter2) = struct.unpack("<LL", f.read(8))
        if 0xFFFFFFFF in (slot1_counter1, slot2_counter1):
            slot1_counter1 = (slot1_counter1 + 1) & 0xFFFFFFFF
            slot2_counter1 = (slot2_counter1 + 1) & 0xFFFFFFFF
        if 0xFFFFFFFF in (slot1_counter2, slot2_counter2):
            slot1_counter2 = (slot1_counter2 + 1) & 0xFFFFFFFF
            slot2_counter2 = (slot2_counter2 + 1) & 0xFFFFFFFF
        base_ofs = storage_start
        if (slot2_counter1, slot2_counter2) > (slot1_counter1, slot1_counter2):
            base_ofs += 0x40000
