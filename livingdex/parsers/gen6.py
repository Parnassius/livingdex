...
def parse(save: Path) -> list[list[str]] | None:
    if save.stat().st_size not in (0x65600, 0x76000):
        return None

    boxes = []
    with save.open("rb") as f:
        f.seek(-0x1F0, os.SEEK_END)
        if f.read(4) != b"FEEB":
            return None

        for box_index in range(31):
            f.seek(-0x43000 + box_index * 0x1B30, os.SEEK_END)
            box = []
            for _ in range(30):
                ekey, flags, checksum, *data = struct.unpack("<LHH112H", f.read(0xE8))
                key = ekey
                for i in range(len(data)):
                    key = (0x41C64E6D * key) + 0x00006073
                    data[i] = data[i] ^ ((key >> 16) & 0xFFFF)

                if ekey == 0 and flags == 0:
                    pokemon = ""
                else:
                    sections = _sections_positions[((ekey >> 13) & 0x1F) % 24]
                    is_egg = (data[sections[1] * 28 + 27] >> 14) & 1
                    if is_egg:
                        pokemon = "egg"
                    else:
                        pokemon_info = _indices[data[sections[0] * 28] & 0xFFFF]
                        if isinstance(pokemon_info, tuple):
                            form = pokemon_info[1][data[sections[0] * 28 + 10] >> 11]
...
_indices: dict[int, str | tuple[str, list[str]]] = {
    ...
    0x29B: "litleo",
    0x29C: "pyroar",
    0x29D: ("flabebe", ["red", "yellow", "orange", "blue", "white"]),
    0x29E: ("floette", ["red", "yellow", "orange", "blue", "white", "eternal"]),
    0x29F: ("florges", ["red", "yellow", "orange", "blue", "white"]),
    0x2A0: "skiddo",
    0x2A1: "gogoat",
    0x2A2: "pancham",
    0x2A3: "pangoro",
    ...
    0x2C4: "phantump",
    0x2C5: "trevenant",
    0x2C6: ("pumpkaboo", ["average", "small", "large", "super"]),
    0x2C7: ("gourgeist", ["average", "small", "large", "super"]),
    0x2C8: "bergmite",
    0x2C9: "avalugg",
    0x2CA: "noibat",
    0x2CB: "noivern",
